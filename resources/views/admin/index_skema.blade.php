...
<div class="flex justify-between items-center mb-8">
    <div>
        <h3 class="text-2xl font-bold text-[#1A3636]">Data Skema</h3>
        <p class="text-sm text-gray-500 mt-1">Kelola data Klasifikasi Skema Usaha (KPS atau KTH).</p>
    </div>
        ...
        <table class="w-full text-sm text-left">
            <thead>
                <tr class="bg-gray-50/50 text-gray-400 font-bold uppercase tracking-widest text-[10px] border-b border-gray-100">
                    ...
                    <th class="py-5 px-8">Kode Skema</th>
                    <th class="py-5 px-8">Nama Skema</th>
                    <th class="py-5 px-8">Jenis Kelompok Binaan</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/80 transition-all duration-200 group">
                    ...
                    <td class="py-5 px-8 font-mono text-xs text-gray-700">{{ $skema->skema_id }}</td>
                    <td class="py-5 px-8 font-bold text-gray-700 group-hover:text-[#4F6F52] transition-colors">{{ $skema->nama_skema }}</td>
                    <td class="py-5 px-8">
                        <span class="px-3 py-1 rounded-full text-xs font-semibold {{ $skema->jenis_kelompok_binaan === 'KPS' ? 'bg-green-100 text-green-700' : 'bg-[#EBF4F6] text-[#088395]' }}">
                            {{ $skema->jenis_kelompok_binaan }}
                        </span>
                    </td>
                    <td class="py-5 px-8">
                        <div class="flex justify-center gap-3">
                            <button onclick="openEditModal({{ json_encode($skema) }})"
                            ...
                            <button onclick="openDeleteModal('{{ $skema->skema_id }}', '{{ $skema->nama_skema }}')"
...
<div id="modalTambah" class="fixed inset-0 z-[70] hidden flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden transform transition-all duration-300 scale-95 opacity-0 modal-content">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            <h3 class="font-bold text-[#1A3636]">Tambah Skema Baru</h3>
        ...
        <form action="{{ route('skema.store') }}" method="POST" class="p-8 space-y-5">
            @csrf
            <div>
                <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Nama Skema</label>
                <input type="text" name="nama_skema" required placeholder="Masukkan nama skema" class="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-[#4F6F52]/10 focus:border-[#4F6F52] outline-none transition-all placeholder:text-gray-300">
            </div>
            <div>
                <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Jenis Kelompok Binaan</label>
                <select name="jenis_kelompok_binaan" required class="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-[#4F6F52]/10 focus:border-[#4F6F52] outline-none transition-all">
                    <option value="">-- Pilih Jenis --</option>
                    <option value="KPS">KPS (Kemitraan Perhutanan Sosial)</option>
                    <option value="KTH">KTH (Kelompok Tani Hutan)</option>
                </select>
            </div>
...
<div id="modalEdit" class="fixed inset-0 z-[70] hidden flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden transform transition-all duration-300 scale-95 opacity-0 modal-content">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            <h3 class="font-bold text-[#1A3636]">Edit Data Skema</h3>
        ...
        <form id="formEdit" action="" method="POST" class="p-8 space-y-5">
            ...
            <div>
                <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Nama Skema</label>
                <input type="text" name="nama_skema" id="edit_nama_skema" required class="w-full px-5 py-3 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all">
            </div>
            <div>
                <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Jenis Kelompok Binaan</label>
                <select name="jenis_kelompok_binaan" id="edit_jenis_kelompok_binaan" required class="w-full px-5 py-3 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all">
                    <option value="">-- Pilih Jenis --</option>
                    <option value="KPS">KPS (Kemitraan Perhutanan Sosial)</option>
                    <option value="KTH">KTH (Kelompok Tani Hutan)</option>
                </select>
...
<div id="modalDelete" class="fixed inset-0 z-[70] hidden flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden transform transition-all duration-300 scale-95 opacity-0 modal-content">
        <div class="p-8 text-center">
            ...
            <h3 class="text-xl font-bold text-gray-800 mb-2">Hapus Skema?</h3>
            <p class="text-gray-500 text-sm mb-8">Apakah Anda yakin ingin menghapus skema <span id="delete_name" class="font-bold text-gray-800"></span>? Tindakan ini tidak dapat dibatalkan.</p>
...
<script>
    ...
    function openEditModal(skema) {
        const form = document.getElementById('formEdit');
        form.action = `/admin/skema/${skema.skema_id}`;
        
        document.getElementById('edit_nama_skema').value = skema.nama_skema;
        document.getElementById('edit_jenis_kelompok_binaan').value = skema.jenis_kelompok_binaan;
